...
from util.NO4K_dataset import NO4K_dataset
from util.augmentation import RandomFlip, RandomCrop, RandomCropOut, RandomBrightness, RandomNoise
from util.util import compute_results
from sklearn.metrics import confusion_matrix
from torch.utils.tensorboard import SummaryWriter
from model.MMFSeg.MMFSeg import EncoderDecoder as MMFSeg
...
############################################################################################# 
parser.add_argument('--f1_2_f2', '-f1', type=bool, default=False)
parser.add_argument('--conv_model_name', '-cm', type=str, default='ConvNeXt_0')
parser.add_argument('--trans_model_name', '-tm', type=str, default='mit_b0')
parser.add_argument('--model_name', '-m', type=str, default='C2T_Conv0_Trans0')
parser.add_argument('--pre_weight', '-prw', type=str, default='/pretrained/mit_b0.pth')


#############################################################################################
parser.add_argument('--weight_name', '-w', type=str, default='C2T_Conv0_Trans0') 
...
parser.add_argument('--data_dir', '-dr', type=str, default='./Pothole/')  
...
def local_loss(locals,label):

    label = torch.cuda.FloatTensor(label.unsqueeze(1).cpu().numpy())
    label = F.interpolate(label, size=locals[0].shape[2:], mode="nearest")
    label = torch.cuda.LongTensor(label.squeeze(1).cpu().numpy())
 
    loss0 = F.cross_entropy(locals[0], label)
    loss1 = F.cross_entropy(locals[1], label)
    loss2 = F.cross_entropy(locals[2], label)
    loss3 = F.cross_entropy(locals[3], label)
...
if __name__ == '__main__':
    ...
    config.trans_model_name = args.trans_model_name # Remember change the path below.

    criterion = nn.CrossEntropyLoss(reduction='mean', ignore_index=config.background)

    model = MMFSeg(cfg=config, norm_layer=nn.BatchNorm2d, Trans_encoder_name=args.trans_model_name, Conv_encoder_name=args.conv_model_name, f1_2_f2=args.f1_2_f2)
    ...
    # preparing folders
    if os.path.exists("./C2T_Conv0_Trans0"):
        shutil.rmtree("./C2T_Conv0_Trans0")
    weight_dir = os.path.join("./C2T_Conv0_Trans0", args.model_name)
    os.makedirs(weight_dir)

    os.chmod(weight_dir, stat.S_IRWXO)  # allow the folder created by docker read, written, and execuated by local machine
    writer = SummaryWriter("./C2T_Conv0_Trans0/tensorboard_log")
    os.chmod("./C2T_Conv0_Trans0/tensorboard_log", stat.S_IRWXO)  # allow the folder created by docker read, written, and execuated by local machine
    os.chmod("./C2T_Conv0_Trans0", stat.S_IRWXO)

    print('training %s on GPU #%d with pytorch' % (args.model_name, args.gpu))
    print('from epoch %d / %s' % (args.epoch_from, args.epoch_max))
    print('weight will be saved in: %s' % weight_dir)

    train_dataset = NO4K_dataset(data_dir=args.data_dir, split='train', transform=augmentation_methods,input_h=config.image_height, input_w=config.image_width)
    val_dataset  = NO4K_dataset(data_dir=args.data_dir, split='val',input_h=config.image_height, input_w=config.image_width)
    test_dataset = NO4K_dataset(data_dir=args.data_dir, split='test',input_h=config.image_height, input_w=config.image_width)

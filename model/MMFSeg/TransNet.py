...
class EncoderDecoder(nn.Module):
    def __init__(self,cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), encoder_name='mit_b2', decoder_name='MLPDecoder', n_class=2, norm_layer=nn.BatchNorm2d):
        ...
        if encoder_name == 'mit_b5':
            ...
            from model.TCNet.encoders.dual_segformer import mit_b5 as backbone
            print("chose mit_b5")
            self.backbone = backbone(norm_fuse=norm_layer)
        elif encoder_name == 'mit_b4':
            #logger.info('Using backbone: Segformer-B4')
            from model.TCNet.encoders.dual_segformer import mit_b4 as backbone
            print("chose mit_b4")
            self.backbone = backbone(norm_fuse=norm_layer)
        elif encoder_name == 'mit_b3':
            #logger.info('Using backbone: Segformer-B4')
            from model.TCNet.encoders.dual_segformer import mit_b3 as backbone
            print("chose mit_b3")
            self.backbone = backbone(norm_fuse=norm_layer)
        elif encoder_name == 'mit_b2':
            #logger.info('Using backbone: Segformer-B2')
            from model.TCNet.encoders.dual_segformer import mit_b2 as backbone
            print("chose mit_b2")
            self.backbone = backbone(norm_fuse=norm_layer)
        elif encoder_name == 'mit_b1':
            #logger.info('Using backbone: Segformer-B1')
            from model.TCNet.encoders.dual_segformer import mit_b1 as backbone
            print("chose mit_b1")
            self.backbone = backbone(norm_fuse=norm_layer)
        elif encoder_name == 'mit_b0':
            #logger.info('Using backbone: Segformer-B0')
            self.channels = [32, 64, 160, 256]
            from model.TCNet.encoders.dual_segformer import mit_b0 as backbone
            print("chose mit_b0")
            self.backbone = backbone(norm_fuse=norm_layer)
        else:
            #logger.info('Using backbone: Segformer-B2')
